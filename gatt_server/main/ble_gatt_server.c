...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            ble_data_recv_handle(param->write.value, param->write.len);

            //esp_log_buffer_hex(GATTS_TAG, param->write.value, param->write.len); // log hexa
...
void app_ble_send_data(uint8_t* data, uint16_t len)
{
    ...
    esp_ble_gatts_send_indicate(ble_server.gatts_if, ble_server.param->write.conn_id, 

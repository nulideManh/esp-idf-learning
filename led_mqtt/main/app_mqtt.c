...
static mqtt_handle_t mqtt_handle_event = NULL;
static mqtt_handle_t mqtt_handle_duty = NULL;
static mqtt_handle_t mqtt_handle_rgb = NULL;
...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event)
{
    ...
    switch (event->event_id) {
        ...
        case MQTT_EVENT_DATA:
            ...
            mqtt_handle_event(event->data, event->data_len);
...
char* json_gen_control(json_gen_test_result_t *result, bool status, char* mode, int direction, int speed, int intensity) 
{    
    char buf[20];
    ...
    json_gen_obj_set_string(&jstr, "mode", mode);
    json_gen_obj_set_int(&jstr, "direction", direction);
    json_gen_obj_set_int(&jstr, "speed", speed);
...
{    
    char buf[20];
    json_gen_test_result_t res;
    memset(result, 0, sizeof(json_gen_test_result_t));
	json_gen_str_t jstr;
    char *tmp = json_gen_control(&res, status, mode, direction, speed, intensity);
	json_gen_str_start(&jstr, buf, sizeof(buf), flush_str, result);

    json_gen_start_object(&jstr);
    json_gen_obj_set_string(&jstr, "deviceId", deviceID);
    json_gen_push_object_str(&jstr, "control", tmp);
    json_gen_end_object(&jstr);
...
{
    if(cb){
        mqtt_handle_event = cb;
    }
}

void mqtt_set_led_duty_callback(void *cb)
{
    if(cb){
        mqtt_handle_duty = cb;
    }
...
void mqtt_set_rgb_callback(void *cb)
{
    if(cb){
        mqtt_handle_rgb = cb;
    }

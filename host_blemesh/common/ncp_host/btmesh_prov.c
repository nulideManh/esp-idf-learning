...
void btmesh_prov_on_event(sl_btmesh_msg_t *evt)
{
  switch (SL_BT_MSG_ID(evt->header)) {
    case sl_btmesh_evt_prov_initialized_id: {
      app_log_warning("sl_btmesh_evt_prov_initialized_id\n");
      ...
      if (0 < networks) {
        ...
        for (size_t i = 0; i < networks; i++) {
          ...
          if (SL_STATUS_OK != sc) {
            ...
            app_log_warning("Create network %d success" APP_LOG_NEW_LINE, current_netkey_index);
          }
        }
    ...
    case sl_btmesh_evt_prov_ddb_list_id: {
      app_log_warning("sl_btmesh_evt_prov_ddb_list_id\n");
      sl_status_t sc;
      ...
      if (SL_STATUS_OK != sc && SL_STATUS_ALREADY_EXISTS != sc) {
        ...
        app_log_warning("Create node 0x%04x in database success" APP_LOG_NEW_LINE, address);
      }
      ...
      if (SL_STATUS_OK != sc && SL_STATUS_ALREADY_EXISTS != sc) {
        ...
        app_log_warning("Add node 0x%04x to network in database!" APP_LOG_NEW_LINE, address);
      }

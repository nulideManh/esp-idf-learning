...
// -----------------------------------------------------------------------------
// Macros
static const char* TAG = "APP CONF";
                                              ...
                                              uint16_t server_address)
{
  ...
  if ((APP_CONF_STATE_IDLE == app_conf.state)
    ...
    if (SL_STATUS_OK == sc) {
      ...
      ESP_LOGI(TAG, "Configuration of node (netkey_idx=%d,addr=0x%04x) is started." NL,
              app_conf.netkey_index,
              app_conf.server_address);
    } else {
            ESP_LOGE(TAG,    "Failed to start dcd get procedure to node "
                                                       ...
                                                       uint16_t server_address)
{
  ...
  if (SILABS_COMPANY_ID == node->dcd.company_id) {
    ESP_LOGI(TAG,   "Node (netkey_idx=%d,addr=0x%04x) runs %s example." NL,
                    netkey_index,
                    server_address,
      ...
      if (SL_STATUS_OK == model_attr_status) {
        ...
        if (attributes & BTMESH_CONF_SIG_MODEL_SUPPORTS_SUBSCRIPTION) {
          ...
          last_task = new_task;
            ...
            ESP_LOGE(TAG,  "Failed to get btmesh model (0x%04x) attributes." NL,
                            model_id->model);
      }
    }
  }
  ...
  switch (app_conf.state) {
    case APP_CONF_STATE_GET_DCD:
    {
      if (BTMESH_CONF_JOB_RESULT_SUCCESS == job->result) {
        ESP_LOGI(TAG, "DCD query of node (netkey_idx=%d,addr=0x%04x) completed." NL,
                app_conf.netkey_index,
                app_conf.server_address);
        ...
        if (SL_STATUS_OK == config_job_status) {
          app_conf_state_transition(APP_CONF_STATE_NODE_CONFIG);
                ...
                ESP_LOGE(TAG,   "Failed to submit configuration job"
                                "(netkey_idx=%d,addr=0x%04x)." NL,
                                app_conf.netkey_index,
                                app_conf.server_address);
        ...
        ESP_LOGE(TAG, "DCD query of node (netkey_idx=%d,addr=0x%04x) failed." NL,
                      app_conf.netkey_index,
                      app_conf.server_address);
        app_conf_state_transition(APP_CONF_STATE_FAILURE);
      }
    ...
    case APP_CONF_STATE_NODE_CONFIG:
      if (BTMESH_CONF_JOB_RESULT_SUCCESS == job->result) {
        ESP_LOGI(TAG, "Configuration of node (netkey_idx=%d,addr=0x%04x) is successful." NL,
                app_conf.netkey_index,
                app_conf.server_address);
        app_conf_state_transition(APP_CONF_STATE_SUCCESS);
      } else {
        ESP_LOGI(TAG, "Configuration of node (netkey_idx=%d,addr=0x%04x) is failed." NL,
                app_conf.netkey_index,
                app_conf.server_address);
        app_conf_state_transition(APP_CONF_STATE_FAILURE);
      }
...
void app_conf_print_nodeinfo_by_addr(uint16_t server_address)
{
  ...
  if (NULL == node) {
    ESP_LOGE(TAG, "Node is not found in btmesh_db." NL);
...
static void app_conf_print_nodeinfo_dcd(const btmesh_db_node_t *const node)
{
  ESP_LOGI(TAG, "Node 0x%04x information:" NL, node->prov.prim_address);
  if (SILABS_COMPANY_ID == node->dcd.company_id) {
    ESP_LOGI(TAG, "Company id: 0x%04x-%s" NL,
            node->dcd.company_id,
            "Silicon Labs");
    ESP_LOGI(TAG, "Product id: 0x%04x-%s" NL,
            node->dcd.product_id,
            btmesh_example_product_id_to_string(node->dcd.product_id));
  } else {
    ESP_LOGI(TAG, "Company id: 0x%04x" NL, node->dcd.company_id);
    ESP_LOGI(TAG, "Product id: 0x%04x" NL, node->dcd.product_id);
  }
  ESP_LOGI(TAG, "Version id: 0x%04x" NL, node->dcd.version_id);
  ESP_LOGI(TAG, "Min replay prot list length: %u" NL, node->dcd.min_replay_prot_list_len);
  ESP_LOGI(TAG, "Features:" NL);
  ESP_LOGI(TAG, "  -Relay:  %u" NL, node->dcd.feature_relay);
  ESP_LOGI(TAG, "  -Proxy:  %u" NL, node->dcd.feature_proxy);
  ESP_LOGI(TAG, "  -Friend: %u" NL, node->dcd.feature_friend);
  ESP_LOGI(TAG, "  -LPN:    %u" NL, node->dcd.feature_low_power);
  ESP_LOGI(TAG, "Elements: (count=%u)" NL, node->dcd.element_count);
  for (uint16_t elem_idx = 0; elem_idx < node->dcd.element_count; elem_idx++) {
    const btmesh_db_element_t *const element = &node->dcd.elements[elem_idx];
    ESP_LOGI(TAG, "  -Element %u:" NL, elem_idx);
    ESP_LOGI(TAG, "    -Address: 0x%04x" NL, element->address);
    ESP_LOGI(TAG, "    -Location: %u" NL, element->location);
    ESP_LOGI(TAG, "    -Models: (count=%u)" NL, element->model_count);
    for (uint16_t mdl_idx = 0; mdl_idx < element->model_count; mdl_idx++) {
      ...
      if (MESH_SPEC_VENDOR_ID == model_id->vendor) {
        // SIG model
        ESP_LOGI(TAG, "      -SIG model: 0x%04x-%s" NL,
                model_id->model,
                btmesh_conf_sig_model_id_to_string(model_id->model));
      } else {
        // Vendor model
        ESP_LOGI(TAG, "      -Vendor (0x%04x) model: 0x%04x" NL,
                model_id->vendor,
                model_id->model);
      }
    }

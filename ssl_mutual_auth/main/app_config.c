...
provision_type_t provisition_type = PROVISION_ACCESSPOINT;
...
void app_config(void)
{
    ...
    if(!provisioned)
    {
        if(provisition_type == PROVISION_SMARTCONFIG) // smart config
        {
            ESP_ERROR_CHECK(esp_wifi_start() );
        ...
        else if(provisition_type == PROVISION_ACCESSPOINT) // access point
        {
            ap_start();

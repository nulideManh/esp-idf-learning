...
int I2C_WriteData(uint8_t slaveAddr, uint8_ regAddr, uint8_t *pData, uint16_t dataLen)
{
    int ret;
...
int I2C_ReadData(uint8_t slaveAddr, uint8_ regAddr, uint8_t *pData, uint16_t dataLen)
{
    int ret;
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
...
static void i2c_test_task(void *arg)
{
    ...
    while(1) {
        ESP_LOG(TAG, "test cnt: %d", cnt++);
        lux = BH1750_ReadLightIntensity();
        xSermaphoreTake(print_mux, portMAX_DELAY);
        printf("sensor value: %.02f [Lux]\n", lux);
        xSemaphoreTake(print_mux);
